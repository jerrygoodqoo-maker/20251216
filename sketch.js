...
let gameState = 'MOVING'; // MOVING, ASKING, ANSWERED, FINALE
...
// 攝影機/視角 物件
let camera = { x: 0, y: 0 };
let moonwalkMode = true; // 預設為月球漫步模式，按下 M 鍵切換
...
function draw() {
  ...
  if (celestialEffect.isActive) {
    ...
    // 當玩家進入圈圈時，觸發結局
    if (playerDist < celestialEffect.radius) {
      startFinale();
    }
  }
...
function drawPlayer() {
  ...
  // 2. 檢查是否在左右移動 (且不在問答中)
  } else if (gameState === 'MOVING' && (keyIsDown(LEFT_ARROW) || keyIsDown(RIGHT_ARROW))) {
  ...
  if (player.direction !== 0) { // 只要有方向性就翻轉
...
function movePlayer() {
  ...
  if ((keyIsDown(LEFT_ARROW) || keyIsDown(RIGHT_ARROW)) && moonwalkMode === false) {
...
function checkInteractions() {
  // --- 檢查是否所有提問者都已完成 ---
  const allSatisfied = questioners.every(q => q.correctAnswersCount >= 2);
  ...
  for (let q of questioners) {
    ...
    if (q.correctAnswersCount >= 2) {
      ...
      // 如果提問者未完成，靠近時觸發提問
      if (d < playerSize + questionerSize && millis() > q.cooldownUntil && unaskedQuestions.length > 0) {
        gameState = 'PROMPT';
        activeQuestioner = q;
        questionPrompt = "挑戰者，你敢回答我的問題嗎？(Y/N)";
...
function drawFinale() {
  ...
  // 顏色恢復後才允許移動，且畫面未靜止
  if (timePassed > fadeDuration && !celestialEffect.isFrozen) {
    movePlayerInFinale();
  } else {
  ...
  // --- 3. 繪製夢幻 ---
  const tempSpeech = hintGiverSpeech;
  ...
  if (dToHintGiver < fleeRadius && !celestialEffect.isFrozen) {
    // 計算逃跑方向 (從皮卡丘指向夢幻)
    let fleeVec = createVector(hintGiver.x - player.x, hintGiver.y - player.y);
    fleeVec.normalize();
    // 將夢幻的速度方向設定為逃跑方向
    const fleeSpeed = random(2.0, 3.5); // 逃跑速度帶有波動
  ...
  // 更新動畫幀
  if (!celestialEffect.isFrozen && (player.isMoving || frameCount % player.animation.frameDelay === 0)) {
    player.animation.currentFrame = (player.animation.currentFrame + 1) % anim.totalFrames;
  }
  let frameX = player.animation.currentFrame * anim.w;
  imageMode(CENTER);
  ...
  // --- 6. 檢查勝利條件並顯示 UI ---
  if (dToHintGiver < playerPikaSize + hintGiverSize || celestialEffect.isFrozen) {
...
function displayPrompt() {
  ...
  textSize(22);
  textAlign(CENTER, CENTER);
  text("是 (Y)", yesButton.x + yesButton.w / 2, yesButton.y + yesButton.h / 2);

  // 繪製 '否' 按鈕
  fill(onNo ? '#F44336' : '#FF5722'); // 紅色系
  rect(noButton.x, noButton.y, noButton.w, noButton.h, 10);
  fill(255);
  text("否 (N)", noButton.x + noButton.w / 2, noButton.y + noButton.h / 2);
}
...
function promptKeyPressed() {
  if (gameState === 'PROMPT') {
    ...
    if (key.toUpperCase() === 'Y') {
      ...
      gameState = 'ASKING'; // 立刻進入問答狀態
      questionPrompt = null; // 隱藏詢問框

      // 1.5 秒後清除提問者的反應，避免一直顯示
      setTimeout(() => {
    ...
    // 玩家選擇 "否"
      ...
      gameState = 'MOVING';
      activeQuestioner.reactionText = "孬種，有種回答！";
      activeQuestioner.reaction = 'wrong'; // 顯示失望動畫
      activeQuestioner.cooldownUntil = millis() + 3000; // 提問者冷卻 3 秒
      questionPrompt = null;
    }
  }
}
...
function displayWinScreen() {
  ...
  textSize(24);
  text("恭喜你完成了所有挑戰！", width / 2, height / 2 - 30);

  // 繪製 "再玩一次？"
  fill(255);
  textSize(28);
  text("要再玩一次嗎？", width / 2, height / 2 + 40);
  ...
  const onNo = mouseX > playAgainNoButton.x && mouseX < playAgainNoButton.x + playAgainNoButton.w &&
               mouseY > playAgainNoButton.y && mouseY < playAgainNoButton.y + playAgainNoButton.h;
  ...
  fill(255);
  textSize(22);
  text("是", playAgainYesButton.x + playAgainYesButton.w / 2, playAgainYesButton.y + playAgainYesButton.h / 2);

  // 繪製 '否' 按鈕
  fill(onNo ? '#F44336' : '#FF5722');
  rect(playAgainNoButton.x, playAgainNoButton.y, playAgainNoButton.w, playAgainNoButton.h, 10);
  fill(255);
  text("否", playAgainNoButton.x + playAgainNoButton.w / 2, playAgainNoButton.y + playAgainNoButton.h / 2);
}
...
function displayTimedHint() {
  if (millis() < hintTimer) {
    ...
    let boxX;
    let buffer = 10;
    let boxY = hintGiver.y - 100; // 顯示在提示者上方

    // 確保提示框不會超出螢幕左右邊界
    if (hintGiver.x < width / 2) {
      // 提示者在左側，對話框放右邊
      boxX = hintGiver.x + (boxW / 2) + 20;
       // Adjust if bubble exceeds right boundary
      if (boxX + boxW/2 > width - buffer) {
        boxX = width - buffer - boxW/2;
      }
    } else {
      // 提示者在右側，對話框放左邊
      boxX = hintGiver.x - (boxW / 2) - 20;
